        ...
        <div className="mx-auto max-w-5xl space-y-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {quickCategories.map((category) => {
              ...
              return (
                <button key={category.label} type="button" onClick={() => setActiveCategory(category.key)} className={`rounded-2xl border p-5 backdrop-blur-xl transition hover:-translate-y-0.5 ${isActive ? 'border-primary/30 bg-gradient-to-br from-primary/20 to-accent/15' : 'border-white/15 bg-white/5'}`}>
                  <div className="mb-8 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-white/10"><span className={`inline-flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br ${category.iconClass}`}><Icon className="h-4 w-4 text-white" /></span></div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => setShowMap((p) => !p)} className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs text-foreground"><MapPinned className="h-3.5 w-3.5" />{showMap ? 'Hide Map' : 'Map View'}</button>
          </div>
          {showMap && <EssentialsMap locations={filteredLocations} />}
          <div className="space-y-3">
            {filteredLocations.map((location) => (
              <article key={location.name} className="rounded-xl border border-white/15 bg-white/5 p-3 backdrop-blur-xl">
                <div className="mb-2 flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-2"><h3 className="text-sm font-semibold">{location.name}</h3><span className="rounded-full bg-green-500/20 px-2 py-0.5 text-[10px] text-green-400">{location.tag}</span></div>
                    <p className="mt-1 text-xs text-muted-foreground">{location.distance} • {location.cost}</p>
                    <p className="mt-1 text-[11px] text-emerald-400">● {location.note}</p>
                  </div>
                  <button type="button" onClick={() => openLocationMap(location.mapsQuery)} className="rounded-md bg-white/10 px-2 py-1 text-[10px]">Details</button>
                </div>
